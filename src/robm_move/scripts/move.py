...
class Move:
    ...
    def odom_callback(self, odom):
        ...
        d = sqrt( delta_x * delta_x + delta_y * delta_y)   # TODO
        
        # TODO: Calculer le cap à suivre
        
        angle_cap = atan2(delta_y , delta_x) 
        erreur = (angle_cap - theta)
        ...
        K = 0.5
        w = erreur * K        # TODO: Calculer la vitesse angulaire du robot (cf. control_heading.py)
        ...
        if((angle_cap) > pi/2.0):
            coef = 0
        elif((angle_cap) < pi/4.0):
            coef = 1
        else:
            coef = (-4.0/pi) * (angle_cap) + 2.0
